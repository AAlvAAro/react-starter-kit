...
module Tiktok
  class MessageTemplatesGenerator
    ...
    SYSTEM_PROMPT = <<~PROMPT
      You are an expert at crafting personalized direct messages and opening lines for professional networking. Generate message templates to help someone reach out to a TikTok creator.
      ...
      **IMPORTANT: Respond entirely in %{language}. All text values in the JSON must be in %{language}.**
      ...
      - Reference specific details from their bio when possible
    ...
    def generate
      ...
      Rails.logger.error("Failed to generate TikTok message templates: #{e.message}")
      nil
    end
    ...
    def user_prompt
      ...
      <<~PROMPT
        Generate personalized DM templates for reaching out to this TikTok creator:
